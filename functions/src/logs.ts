...
import { Client, MessageEmbed, User, VoiceChannel, VoiceState } from 'discord.js'
...
interface VoiceMember {
	...
	channelName: string,
	userID: string,
	state: VoiceStateCustom	
}
interface VoiceStateCustom {
	selfMute: boolean,
	selfDeaf: boolean,
	serverMute: boolean,
	serverDeaf: boolean,
...
class VoiceMember {
    constructor(user: User, channel: VoiceChannel, client: Client, logChannel: string) {
        this.client = client
    ...
    changeChannelEmbed(newChannel: VoiceChannel): void {
    ...
    changeStateEmbed(message:string): void {
        const embed = new MessageEmbed({
            color: 15844367,
    ...
    compareDifference(newMember: VoiceState) {
        const temp = {
        ...
        if (this.state.selfDeaf !== temp.selfDeaf) this.updateState(updateStateCode.selfDeaf)
        if (this.state.selfMute !== temp.selfMute) this.updateState(updateStateCode.selfMute)
        if (this.state.serverDeaf !== temp.serverDeaf) this.updateState(updateStateCode.serverDeaf)
        if (this.state.serverMute !== temp.serverMute) this.updateState(updateStateCode.serverMute)
        if (this.state.streaming !== temp.streaming) this.updateState(updateStateCode.streaming)
    }
    joinEmbed(): void {
        const embed = new MessageEmbed({
            color: 3066993,
